...
export function Banner({ defaultOpen, onDismiss }: Props) {
  ...
  return (
    <div className="relative full text-xs border border-dashed border-amber-500/60 bg-amber-500/10 text-foreground py-2 pl-2 pr-8">
      ...
      <button
        ...
        className="absolute top-2 right-2 text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 transition-colors cursor-pointer"
        onClick={() => {
          onDismiss()
          setOpen(false)
        }}
